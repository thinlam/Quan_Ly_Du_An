...
/// </summary>
public record DeXuatChuTruongMoiTraLaiCommand(Guid Id, string NoiDung) : IRequest<int>;

internal class DeXuatChuTruongMoiTraLaiCommandHandler : IRequestHandler<DeXuatChuTruongMoiTraLaiCommand, int> {
    ...
    public DeXuatChuTruongMoiTraLaiCommandHandler(IServiceProvider serviceProvider) {
    ...
    public async Task<int> Handle(DeXuatChuTruongMoiTraLaiCommand request, CancellationToken cancellationToken) {
        ...
        var trangThaiTraLai = await _statusRepository.GetQueryableSet(OnlyUsed: true, OnlyNotDeleted: true, OrderByIndex: false)
            .FirstOrDefaultAsync(s => s.Ma == TrangThaiPheDuyetCodes.DeXuatChuTruongMoi.TraLai && s.Loai == PheDuyetEntityNames.DeXuatChuTruongMoi, cancellationToken);
