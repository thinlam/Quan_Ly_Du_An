...
/// </summary>
public record DeXuatNhuCauKinhPhiDuyetCommand(Guid Id) : IRequest<int>;

internal class DeXuatNhuCauKinhPhiDuyetCommandHandler : IRequestHandler<DeXuatNhuCauKinhPhiDuyetCommand, int> {
    ...
    public DeXuatNhuCauKinhPhiDuyetCommandHandler(IServiceProvider serviceProvider) {
    ...
    public async Task<int> Handle(DeXuatNhuCauKinhPhiDuyetCommand request, CancellationToken cancellationToken) {
        ...
        var trangThaiDaDuyet = await _statusRepository.GetQueryableSet(OnlyUsed: true, OnlyNotDeleted: true, OrderByIndex: false)
            .FirstOrDefaultAsync(s => s.Ma == TrangThaiPheDuyetCodes.DeXuatMacDinh.DaDuyet && s.Loai == PheDuyetEntityNames.DeXuatNhuCauKinhPhi, cancellationToken);
