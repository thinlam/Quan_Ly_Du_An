...
/// </summary>
public record ToTrinhKeHoachTrinhCommand(Guid Id, string? NoiDung = null) : IRequest<int>;

internal class ToTrinhKeHoachTrinhCommandHandler : IRequestHandler<ToTrinhKeHoachTrinhCommand, int> {
    private readonly IRepository<ToTrinhKeHoach, Guid> _repository;
    private readonly IRepository<PheDuyetHistory, Guid> _historyRepository;
    private readonly IRepository<DanhMucTrangThaiPheDuyet, int> _statusRepository;
    private readonly IUserProvider _userProvider;
    private readonly IUnitOfWork _unitOfWork;

    public ToTrinhKeHoachTrinhCommandHandler(IServiceProvider serviceProvider) {
        _repository = serviceProvider.GetRequiredService<IRepository<ToTrinhKeHoach, Guid>>();
    ...
    public async Task<int> Handle(ToTrinhKeHoachTrinhCommand request, CancellationToken cancellationToken) {
        ...
        var trangThaiTraLai = await _statusRepository.GetQueryableSet(OnlyUsed: true, OnlyNotDeleted: true, OrderByIndex: false)
            .FirstOrDefaultAsync(s => s.Ma == TrangThaiPheDuyetCodes.ToTrinhKeHoach.TraLai && s.Loai == PheDuyetEntityNames.ToTrinhKeHoach, cancellationToken);
        var trangThaiDaTrinh = await _statusRepository.GetQueryableSet(OnlyUsed: true, OnlyNotDeleted: true, OrderByIndex: false)
            .FirstOrDefaultAsync(s => s.Ma == TrangThaiPheDuyetCodes.ToTrinhKeHoach.DaTrinh && s.Loai == PheDuyetEntityNames.ToTrinhKeHoach, cancellationToken);
        ...
        var history = new PheDuyetHistory {
            Id = Guid.NewGuid(),
            EntityName = PheDuyetEntityNames.ToTrinhKeHoach,
            EntityId = entity.Id,
