...
/// </summary>
public record ThoaThuanGiaoViecDuyetCommand(Guid Id) : IRequest<int>;

internal class ThoaThuanGiaoViecDuyetCommandHandler : IRequestHandler<ThoaThuanGiaoViecDuyetCommand, int> {
    ...
    public ThoaThuanGiaoViecDuyetCommandHandler(IServiceProvider serviceProvider) {
    ...
    public async Task<int> Handle(ThoaThuanGiaoViecDuyetCommand request, CancellationToken cancellationToken) {
        ...
        var trangThaiDaDuyet = await _statusRepository.GetQueryableSet(OnlyUsed: true, OnlyNotDeleted: true, OrderByIndex: false)
            .FirstOrDefaultAsync(s => s.Ma == TrangThaiPheDuyetCodes.KeHoachLuaChonNhaThauRutGon.DaDuyet && s.Loai == PheDuyetEntityNames.KeHoachLuaChonNhaThauRutGon, cancellationToken);
