...
-   **Resource-Level Authorization (Application Layer):** Beyond ASP.NET policy checks, the Application layer enforces row-level authorization via per-request scoped `IAuthorizationContext`. The context exposes cached flags such as `HasKhtcBypass` (true when the user's `PhongBanID` equals `PhongKHTCId`). Authorization providers (`DuAnAuthorizationProvider`, `BuocAuthorizationProvider`) implement `CanExecuteAsync`, `CanViewAsync`, `Filter<T>`, `FilterVisibleSteps`, and `FilterVisibleChildEntities`. When `HasKhtcBypass == true`, these providers short-circuit: `CanExecute`/`CanView` return `true`, filters return the unfiltered query, and `EnsureCanExecute` skip the ownership check entirely. KHTC users therefore gain full read/write across `DuAn` and `DuAnBuoc` without per-record ownership (Lãnh đạo/Phòng ban chính/Phối hợp) checks; non-KHTC users continue to be filtered by the standard ownership rules.
