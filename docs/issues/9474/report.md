...
**Doc mẫu cấu trúc:** `docs/issues/9460/ky-so-crud.md`  
**Trạng thái:** Code + migration **đã có** — **đang chỉnh** dùng chung `Loai = DeXuatMacDinh` (§1.5); sau đó `database update` + build/test (§6).

---

## Quyết định — Dùng chung `Loai` `DeXuatMacDinh` (không tạo `Loai` mới)

| Khái niệm | Giá trị | Dùng cho |
|-----------|---------|----------|
| **Entity phê duyệt (hub)** | `PheDuyetEntityNames.BaoCaoKetQuaKhaoSat` | Route `api/phe-duyet/BaoCaoKetQuaKhaoSat/...`, `PheDuyetHistory.EntityName`, `GET types`, `danh-sach?type=...` |
| **Nhóm trạng thái DM** | `PheDuyetEntityNames.DeXuatMacDinhStt` → chuỗi DB **`DeXuatMacDinh`** | `DmTrangThaiPheDuyet.Loai`, lookup `TrangThaiId` trong Insert/Update/Trinh/Duyet/TraLai |
| **Mã trạng thái** | `TrangThaiPheDuyetCodes.DeXuatMacDinh` (`DT` / `ĐTr` / `ĐD` / `TL`) | Giống các module Đề xuất (Chủ trương, Nhu cầu KP, …) — **không** cần `Loai` riêng `BaoCaoKetQuaKhaoSat` |

**Không seed** thêm 4 dòng `Loai = BaoCaoKetQuaKhaoSat` (Id 26–29). Dùng bản ghi sẵn có **`Loai = DeXuatMacDinh`** (Id **30–33**, `Ma` = `DT` / `ĐTr` / `ĐD` / `TL`, `Stt` 1–4 — giống seed `HasData` bên dưới).

**FE — combobox trạng thái:**

```http
GET /api/danh-muc-trang-thai-phe-duyet/danh-sach-day-du?Loai=DeXuatMacDinh&pageIndex=0&pageSize=50
```

> `Loai` query = cột `DmTrangThaiPheDuyet.Loai`, **không** truyền `BaoCaoKetQuaKhaoSat`.

**Refactor code (sau doc):** `DanhMucTrangThaiPheDuyetConfiguration` (xóa block 26–29), migration mới nếu DB đã insert 26–29, toàn bộ `BaoCaoKetQuaKhaoSat*Command` đổi lookup sang `DeXuatMacDinh` + `DeXuatMacDinhStt`. Có thể **giữ** class `TrangThaiPheDuyetCodes.BaoCaoKetQuaKhaoSat` (cùng mã) hoặc thay bằng `DeXuatMacDinh` — ưu tiên một nguồn (`DeXuatMacDinh`).
...
| Bảng DB | `BaoCaoKetQuaKhaoSat` + FK `TrangThaiId` → `DmTrangThaiPheDuyet` **`Loai = DeXuatMacDinh`** (Id **30–33**, không seed `Loai` mới) |
...
| `TrangThaiPheDuyetCodes.DeXuatMacDinh` | Mã `DT`, `ĐTr`, `ĐD`, `TL` (dùng cho UC55 — **không** `Loai` riêng) |
| `BaoCaoKetQuaKhaoSatConfiguration.cs` | EF: bảng, FK `DuAn`, `DmTrangThaiPheDuyet`, max length 4000 |
| `DanhMucTrangThaiPheDuyetConfiguration.cs` | **Không** seed block `BaoCaoKetQuaKhaoSat`; dùng `DeXuatMacDinh` (30–33) |
| `QLDA.Migrator/Migrations/...` | `CreateTable` `BaoCaoKetQuaKhaoSat`; **không** INSERT 26–29 `Loai=BaoCaoKetQuaKhaoSat` (migration mới nếu đã apply cũ) |
...
| File | Nội dung |
|------|----------|
| `docs/issues/9474/report.md` | Task implement + API contract + checklist |
...
- [ ] Refactor: xóa seed 26–29, handlers → `DeXuatMacDinh` / `DeXuatMacDinhStt` (§1.5)
- [ ] `dotnet ef database update` (migration mới nếu đổi seed / data fix 26–29)
...
| 5 | **Trạng thái DM** | Dùng chung **`Loai = DeXuatMacDinh`** (4 mã `DT/ĐTr/ĐD/TL`, Id **30–33**) — **không** seed `Loai` mới |
...
| `TrangThaiId` | `int?` | `int` FK | → `DanhMucTrangThaiPheDuyet`, **`Loai = DeXuatMacDinh`** (`PheDuyetEntityNames.DeXuatMacDinhStt`) |
...
### 1.5 Quyết định `DmTrangThaiPheDuyet` — dùng chung `DeXuatMacDinh`

UC55 **không** thêm nhóm `Loai = BaoCaoKetQuaKhaoSat` trên `DmTrangThaiPheDuyet` (tránh trùng 4 trạng thái giống Đề xuất). Cùng pattern `DeXuatChuTruongMoi`, `DeXuatNhuCauKinhPhi`, …:

- Lookup trong Application: `s.Ma == TrangThaiPheDuyetCodes.DeXuatMacDinh.* && s.Loai == PheDuyetEntityNames.DeXuatMacDinhStt`
- Hub / lịch sử vẫn `EntityName = PheDuyetEntityNames.BaoCaoKetQuaKhaoSat` (phân biệt nghiệp vụ báo cáo khảo sát)

| Thành phần | Trước (tạm) | Sau (chốt) |
|------------|-------------|------------|
| Seed `HasData` Id 26–29, `Loai=BaoCaoKetQuaKhaoSat` | Có trong config/migration cũ | **Xóa** — dùng Id **30–33**, `Loai=DeXuatMacDinh` |
| `TrangThaiPheDuyetCodes.BaoCaoKetQuaKhaoSat` | Class riêng (cùng mã) | Có thể giữ hoặc gộp — handler **ưu tiên** `DeXuatMacDinh` |
| FE dropdown | `Loai=BaoCaoKetQuaKhaoSat` | **`Loai=DeXuatMacDinh`** |

**DB đã có Id 26–29 `Loai=BaoCaoKetQuaKhaoSat`:** migration/data fix riêng (soft-delete hoặc UPDATE `Loai`) — không sửa migration cũ; tạo migration mới nếu cần.
...
| Trạng thái DM `DeXuatMacDinh` (30–33) | **Có trên DB dev** — **chỉnh doc/code** bỏ seed 26–29 `Loai=BaoCaoKetQuaKhaoSat` |
...
| Entity name + dispatch | Có | `PheDuyetEntityNames.BaoCaoKetQuaKhaoSat`, 3 dispatch; DM **`DeXuatMacDinh`** |
...
**File:** `QLDA.Domain/Constants/TrangThaiPheDuyetCodes.cs` — **không bắt buộc** class `BaoCaoKetQuaKhaoSat`; UC55 dùng sẵn:

```csharp
public static class DeXuatMacDinh
{
...
Kèm `PheDuyetEntityNames.DeXuatMacDinhStt` (`"DeXuatMacDinh"`) khi query `DmTrangThaiPheDuyet`.

> Nếu đã thêm `TrangThaiPheDuyetCodes.BaoCaoKetQuaKhaoSat` (cùng mã) — có thể xóa khi refactor handler; **không** tạo seed `Loai` mới.
...
### Bước 4 – Persistence: `DmTrangThaiPheDuyet` + Migration bảng UC55

**Không** thêm seed `Loai = BaoCaoKetQuaKhaoSat` (Id 26–29). UC55 dùng nhóm **`DeXuatMacDinh`** đã có (vd. Id **30–33** trên DB — `Loai = DeXuatMacDinh`, `Ma` = `DT` / `ĐTr` / `ĐD` / `TL`).

**File:** `QLDA.Persistence/Configurations/DanhMuc/DanhMucTrangThaiPheDuyetConfiguration.cs`

- **Xóa** block comment `// BaoCaoKetQuaKhaoSat statuses` (dòng `Id = 26` … `29`, `Loai = PheDuyetEntityNames.BaoCaoKetQuaKhaoSat`) nếu đã thêm.
- **Đảm bảo** `HasData` cho `DeXuatMacDinh` (nếu chưa có trong repo — thêm Id **30–33**):

```csharp
// DeXuatMacDinh — dùng chung cho Đề xuất + UC55 Báo cáo KQKS (không TC)
...
**Migration bảng `BaoCaoKetQuaKhaoSat`:**

```bash
ef.bat add BaoCaoKetQuaKhaoSat
# Chỉ CreateTable + index/FK — KHÔNG INSERT DmTrangThaiPheDuyet Loai=BaoCaoKetQuaKhaoSat
ef.bat update --sqlite
```

> Nếu migration cũ đã INSERT Id 26–29: migration **mới** (data fix) — không sửa file migration đã apply. Commit: **Domain + Persistence + Migrator** khi đổi seed.
    ...
    : IRequestHandler<BaoCaoKetQuaKhaoSatInsertCommand, BaoCaoKetQuaKhaoSat>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        entity.TrangThaiId = trangThaiDuThao?.Id;  // Id 30 khi seed DeXuatMacDinh
...
> Mọi command workflow (`Update`, `Trinh`, `Duyet`, `TraLai`) đổi tương tự: `TrangThaiPheDuyetCodes.DeXuatMacDinh.*` + `Loai == PheDuyetEntityNames.DeXuatMacDinhStt`. `PheDuyetHistory.EntityName` **giữ** `BaoCaoKetQuaKhaoSat`.
...
[x] 3.  TrangThaiPheDuyetCodes (dùng `DeXuatMacDinh` — có thể bỏ class `BaoCaoKetQuaKhaoSat` khi refactor)
[x] 4.  Tạo BaoCaoKetQuaKhaoSatConfiguration (EF)
[ ] 5.  DM: **không** seed `Loai=BaoCaoKetQuaKhaoSat` (26–29); dùng `DeXuatMacDinh` (30–33); xóa block seed cũ trong Configuration
...
- **`DmTrangThaiPheDuyet`:** `Loai = DeXuatMacDinh` (Id 30–33) — **không** nhóm `Loai = BaoCaoKetQuaKhaoSat`; **không** seed `TC`.
- **Application handlers:** lookup `DeXuatMacDinh` + `DeXuatMacDinhStt`; hub/history vẫn `EntityName = BaoCaoKetQuaKhaoSat`.
...
- Migration UC55 cũ có thể đã INSERT Id 26–29 `Loai=BaoCaoKetQuaKhaoSat` — cần migration/data fix mới (không sửa file cũ). Target: chỉ `DeXuatMacDinh` (30–33).
...
|---|----------|-----|---------|
| 1 | Tạo mới `DuAnId` hợp lệ | `POST .../them-moi` | `TrangThaiId` = DT (`DeXuatMacDinh`, Id **30**) |
| 1b | Combobox trạng thái | `GET .../danh-muc-trang-thai-phe-duyet/danh-sach-day-du?Loai=DeXuatMacDinh` | 4 bản ghi, không gọi `Loai=BaoCaoKetQuaKhaoSat` |
...
| DM trạng thái chung | `DeXuatChuTruongMoiInsertCommand` + `TrangThaiPheDuyetCodes.DeXuatMacDinh` + `DeXuatMacDinhStt` |
| Filter Loai combobox | `docs/issues/dm-trang-thai-phe-duyet-filter-loai/report.md` |
